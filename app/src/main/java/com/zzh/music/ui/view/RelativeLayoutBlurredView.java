 ...
 * @QQ: 1299234582
 * @Author: zzh
 * @Description: Music 模糊背景
 */
public class RelativeLayoutBlurredView extends RelativeLayout {
    /**
     * 模糊最大化值
     */
    ...
    public RelativeLayoutBlurredView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
        initAttr(context, attrs);
    }

    public RelativeLayoutBlurredView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        // blur image
        if (null != drawable) {
            mOriginBitmap = BlurredBitmapUtil.drawableToBitmap(drawable);
            mBlurredBitmap = BlurredBitmapUtil.blur(context, mOriginBitmap);
        }
    ...
    public void setBlurredImg(Bitmap blurredBitmap) {
        if (null != blurredBitmap) {
            ...
            mBlurredBitmap = BlurredBitmapUtil.blur(mContext, blurredBitmap);
            setImageView();
    ...
    public void setBlurredImg(Drawable blurDrawable) {
        if (null != blurDrawable) {
            mOriginBitmap = BlurredBitmapUtil.drawableToBitmap(blurDrawable);
            mBlurredBitmap = BlurredBitmapUtil.blur(mContext, mOriginBitmap);
            setImageView();
            setMove(mContext, isMove);
        }
    }
